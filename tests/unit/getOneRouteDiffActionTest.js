...
describe('getOneRouteDiffAction', () => {
    ...
    describe('generated states with params', () => {
        it('returns simple action for /workspaces to /workspace/1 with generated states', () => {
            const currentState = getStateFromPath('/workspaces', linkinConfig.config);
            const targetState = getStateFromPath('/workspace/1', linkinConfig.config);
            const action = getOneRouteDiffAction(currentState, targetState);

            expect(action).toEqual(CommonActions.navigate({name: 'Workspace_Initail', path: '/workspace/1', params: {policyID: '1'}}));
        });
        it('returns simple action for /workspace/1 to /workspace/1/bills with generated states', () => {
            const currentState = getStateFromPath('/workspaces/1', linkinConfig.config);
            const targetState = getStateFromPath('/workspace/1/bills', linkinConfig.config);
            const action = getOneRouteDiffAction(currentState, targetState);

            expect(action).toEqual(CommonActions.navigate({name: 'Workspace_Initail_Bills', path: '/workspace/1/bills', params: {policyID: '1'}}));
        });
    });
});
